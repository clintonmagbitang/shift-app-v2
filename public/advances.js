(async () => {
  ...
  async function api(path, options = {}) {
    const response = await fetch(`/api/v2/advances${path}`, { ...options, headers: authHeaders({ 'Content-Type': 'application/json' }) });
    let body;
    try { body = await response.json(); } catch { throw new Error('The Advances service is not available yet. Start the configured V2 server.'); }
  ...
  async function load(success = '') {
    ...
    try {
      ...
      for (const tx of data.transactions) {
        ...
        [tx.transaction_date, tx.is_opening ? 'Opening balance' : tx.type === 'advance' ? 'Advance' : 'Repayment', tx.reference || '—', tx.remarks || '—', money(tx.debit), money(tx.credit), money(tx.balance)].forEach((text, index) => {
      ...
      message.textContent = success || (admin ? 'Ledger loaded. You can record an advance or repayment.' : 'Your ledger is read-only.');
