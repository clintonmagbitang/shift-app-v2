...
function render() {
  ...
  for(const row of rows) {
    ...
    for(const [key,label] of fields) {
      ...
      input.oninput=()=>{row.values[key]=input.value;row.dirty=true;tr.classList.add('dirty');row.netCell.textContent=money(net(row));row.status.textContent='Unsaved changes';totals();};td.append(input);
    }
    ...
    const a=document.createElement('a');a.href=`/payroll.html?employee=${row.employee.id}&cutoff=${encodeURIComponent(loadedCutoff)}`;a.textContent='Individual form';a.onclick=e=>{if(rows.some(r=>r.dirty) && !confirm('Unsaved changes will be lost. Open the individual form?'))e.preventDefault();};status.append(a);
  }
  totals();lock(false);
}
async function load() {
  if (busy || (rows.some(r=>r.dirty) && !confirm('Discard unsaved worksheet changes and reload?'))) return;
  ...
  try {
    ...
    render();$('message').textContent=`${rows.length} active employees. Refresh after editing an individual form to see its latest saved values.`;
...
async function save(finalize) {
  ...
  if(!selected.length){$('message').textContent='No changed drafts to save.';return;}
