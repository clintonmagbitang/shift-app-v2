...
(async () => {
  ...
  const base = `http://127.0.0.1:${config.PORT}/api/v2/advances`;
  ...
  try {
    ...
    console.log('PASS: persisted advances/repayments, 1500 balance change, duplicate retry protection, employee isolation and write rejection.');
  } finally {
    await sql`DELETE FROM employee_advance_ledger_v2 WHERE request_id IN (${ids[0]}::uuid, ${ids[1]}::uuid)`;
