...
function advancesRouter({ sql, requireAuth, requireAdmin, enabled }) {
  const router = express.Router();
  router.use(requireAuth);
  router.use((req, res, next) => {
    if (!enabled) return res.status(503).json({ error: 'Advances is waiting for the separate V2 development database to be configured.' });
  ...
  router.get('/', async (req, res) => {
    ...
    try {
      const rows = await sql`
        ...
        FROM (
          SELECT id,user_id,transaction_date,type,amount,reference,remarks,false AS is_opening FROM employee_advance_ledger_v2
          UNION ALL
          ...
          FROM employee_ledger_openings_v2 WHERE ledger = 'advances'
        ) ledger WHERE user_id = ${userId}
        ORDER BY transaction_date, id`;
      res.json({ transactions: rows, balance: rows.length ? rows[rows.length - 1].balance : '0.00' });
    } catch { res.status(500).json({ error: 'Unable to load advances. Check that the V2 migration has been applied.' }); }
  });
  router.post('/opening', requireAdmin, async (req,res) => {
    ...
    try {
      ...
      const rows = await sql`INSERT INTO employee_ledger_openings_v2(user_id,ledger,balance,created_by)
        VALUES(${userId},'advances',${balance},${req.user.id})
        ON CONFLICT(user_id,ledger) DO NOTHING RETURNING id`;
      if (!rows.length) {
        const existing = await sql`SELECT id FROM employee_ledger_openings_v2 WHERE user_id=${userId} AND ledger='advances' AND balance=${balance}::numeric`;
  ...
  router.post('/', requireAdmin, async (req, res) => {
    ...
    try {
      ...
      if (!employees.length) return res.status(400).json({ error: 'Employee not found.' });
      const rows = await sql`
        INSERT INTO employee_advance_ledger_v2 (user_id, transaction_date, type, amount, reference, remarks, created_by, request_id)
      ...
      if (!rows.length) {
        const existing = await sql`SELECT id FROM employee_advance_ledger_v2 WHERE request_id = ${tx.requestId}::uuid
...
module.exports = { advancesRouter, validateTransaction };
